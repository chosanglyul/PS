...
ll ccw(pl a, pl b, pl c, pl d) { d.fi += b.fi-c.fi; d.se += b.se-c.se; return ccw(a, b, d); }
...
int main() {
    ...
    while(t--) {
        ...
        sort(A.begin()+1, A.end(), [&](pl a, pl b) {
            ll cw = ccw(A[0], a, b);
            if(cw) return cw > 0;
        ...
        int a = 0, b = 0, m = hull.size();
        for(int i = 0, j = 0; i < m; i++) {
            while((j+1) != i && ccw(hull[i], hull[(i+1)%m], hull[j], hull[(j+1)%m]) > 0) {
                if(dst(hull[i], hull[j]) > dst(hull[a], hull[b])) a = i, b = j;
                j = (j+1)%m;
            }
            if(dst(hull[i], hull[j]) > dst(hull[a], hull[b])) a = i, b = j;
        }
